...
function CreateNPC(model, coords)
	...
	local npc = CreatePed(mf(5), hashMonel, coords[1], coords[2], coords[3] - 1.0, coords[4], false, true)
	SetEntityHeading(npc, coords[4])
...
function DrawText3D(coords, text)
    ...
    SetTextFont(mf(4))
    SetTextProportional(mf(1))
    SetTextColour(mf(255), mf(255), mf(255), mf(255))
    SetTextEntry("STRING")
    SetTextCentre(true)
    AddTextComponentString(text)
    SetDrawOrigin(coords.x, coords.y, coords.z, mf(0))
    DrawText(0.0, 0.0)
    local factor = (string.len(text)) / mf(370)
    DrawRect(0.0, 0.0 + 0.0125, 0.017 + factor, 0.03, mf(0), mf(0), mf(0), mf(75))
    ClearDrawOrigin()
end
...
function OnKillUpdate(isUpdate,data,isStart)
	...
	if isUpdate then
		...
		if not isStart then
			...
			ESX.ShowNotification('! You need ~y~'..currentGunGame.loadout[data.currentLevel].UpdatesAt - data.currentKills..' more kills to upgrade!')
		end
	end
end
...
function RespawnAtRandomSpawnPoint(isStart)
	...
	local SpawnPoint = TransformVector4(currentGunGame.GunGameRespawns[math.random(1,#currentGunGame.GunGameRespawns)]);
	...
	if isStart then
		...
		GiveWeaponToPed(PlayerPedId(),GetHashKey(currentGunGame.loadout[myGunGameData.currentLevel].weapon),9999,false,true);

		SetPedAmmo(PlayerPedId(),GetHashKey(currentGunGame.loadout[myGunGameData.currentLevel].weapon),9999);

		SetCurrentPedWeapon(PlayerPedId(),GetHashKey(currentGunGame.loadout[myGunGameData.currentLevel].weapon),true);
	end
end
...
function EnteredGunGame(data, mydata,lbdata)
	...
	SendNUIMessage({
		action = 'open-lb',
		myindex = LeaderBoardData[myGunGameData.identifier],
...
function TransformVector4(vector)
	return {x = vector.x, y = vector.y, z = vector.z, heading = vector.w};
end
